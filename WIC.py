...
try:
    ...
    def _dumps(obj): return _json.dumps(obj, option=_json.OPT_INDENT_2).decode()
except Exception:
...
PRESETS: List[ModelPreset] = [
    ...
    ModelPreset(
        ...
        perm_writes={
            20:0,21:0,22:0,23:0, 24:0,25:0,28:0,29:0,
            30:0,34:0, 46:94,47:94,60:94,61:94
        },
...
def eeprom_write_oid(pw_hi: int, pw_lo: int, addr_lo: int, addr_hi: int,
                     value: int, write_key: List[int]) -> str:
...
async def eeprom_read_byte(ip: str, conf: SnmpConf, pw: Tuple[int,int],
                           addr_lo: int, addr_hi: int) -> Optional[int]:
    oid = eeprom_read_oid(pw[0], pw[1], addr_lo, addr_hi)
    data = await snmp_get_octets(ip, oid, conf)
    if not data: return None
    return data[-1]

async def eeprom_write_byte(ip: str, conf: SnmpConf, pw: Tuple[int,int],
                            addr_lo: int, addr_hi: int, value: int, key: List[int]) -> bool:
...
class App(tk.Tk):
    def __init__(self):
        ...
        self._build_ui()
        self.log("Ready. Scan or enter the printer IP (e.g., 192.168.1.6).")

    def _build_ui(self):
        root = ttk.Frame(self, padding=12)
        root.pack(fill="both", expand=True)

        ttk.Button(root, text="Scan", command=self.scan_mdns).grid(row=0, column=0, sticky="w")
        self.dev_var = tk.StringVar()
        self.dev_combo = ttk.Combobox(root, textvariable=self.dev_var, width=80)
        self.dev_combo.grid(row=0, column=1, columnspan=4, sticky="ew", padx=(8,0))

        ttk.Label(root, text="Community").grid(row=1, column=0, sticky="w", pady=(8,0))
        self.community_var = tk.StringVar(value="public")
        ttk.Entry(root, textvariable=self.community_var, width=16).grid(row=1, column=1, sticky="w", pady=(8,0))

        ttk.Button(root, text="Identify", command=self.on_identify).grid(row=1, column=2, sticky="w", padx=(8,0))
        ttk.Button(root, text="Reset (rw quick)", command=self.on_reset_rw).grid(row=1, column=3, sticky="w", padx=(8,0))

        box = ttk.Frame(root); box.grid(row=2, column=0, columnspan=5, sticky="ew", pady=(8,0))
        ttk.Label(box, text="Target used % after reset:").pack(side="left")
        self.pct_spin = ttk.Spinbox(box, from_=0.0, to=90.0, increment=0.5, width=8, textvariable=self.target_pct_var)
        self.pct_spin.pack(side="left", padx=(6,14))
        ttk.Checkbutton(box, text="Refresh maintenance thresholds to 94", variable=self.thresh_enable_var).pack(side="left")

        ttk.Label(root, text="Progress").grid(row=3, column=0, sticky="w", pady=(12,0))
        self.prog = ttk.Progressbar(root, maximum=100)
        self.prog.grid(row=3, column=1, columnspan=4, sticky="ew", pady=(12,0))

        bar = ttk.Frame(root); bar.grid(row=4, column=0, columnspan=5, sticky="ew", pady=(10,6))
        ttk.Button(bar, text="Read waste counters", command=self.on_read_counters).pack(side="left")
        ttk.Button(bar, text="Permanent EEPROM reset to %", command=self.on_perm_reset).pack(side="left", padx=8)
        ttk.Button(bar, text="Detect keys (quick)", command=self.on_detect_keys).pack(side="left", padx=8)
        ttk.Button(bar, text="Load WIC log", command=self.on_load_wic).pack(side="left", padx=8)
        ttk.Button(bar, text="Printer Status (ST2)", command=self.on_status).pack(side="left", padx=8)
        ttk.Button(bar, text="Save Log", command=self.on_save).pack(side="right")

        info = ttk.Frame(root); info.grid(row=5, column=0, columnspan=5, sticky="ew", pady=(6,8))
        self.lbl_model = ttk.Label(info, text="Model: Unknown"); self.lbl_model.pack(side="left")
        self.lbl_keys = ttk.Label(info, text=" Keys: pw=n/a write=n/a divider=n/a"); self.lbl_keys.pack(side="left", padx=(16,0))
    ...
    def set_progress(self, v: int):
        self.prog["value"] = v; self.update_idletasks()
    ...
    def _update_labels(self):
        pw = f"{self.password[0]}.{self.password[1]}" if self.password else "n/a"
        wk = "yes" if self.write_key else "n/a"
        div = f"{self.divider:.2f}" if self.divider else "n/a"
        self.lbl_model.config(text=f"Model: {self.model_label}")
        self.lbl_keys.config(text=f" Keys: pw={pw} write={wk} divider={div}")

    def _apply_presets(self, label_lower: str):
        for p in PRESETS:
            if any(tok in label_lower for tok in p.match):
                if not self.password and p.password: self.password = p.password
                if not self.write_key and p.write_key: self.write_key = p.write_key[:]
                if not self.divider and p.divider: self.divider = p.divider
                if not self.addr_pairs and p.addr_pairs: self.addr_pairs = p.addr_pairs[:]
    ...
    def on_identify(self):
        ...
        if not ip:
            messagebox.showerror("No printer", "Select or type the printer IP."); return
        self.snmp.community = self.community_var.get().strip() or "public"
        self.set_progress(0)
        self.log(f"Connecting to printer {ip}…")
        threading.Thread(target=self._identify_thread, args=(ip,), daemon=True).start()
    ...
    def _identify_thread(self, ip: str):
        try:
            ...
            self.set_progress(0); self.log(f"[!] Identify error: {e!r}")
    ...
    def on_reset_rw(self):
        ...
        threading.Thread(target=self._reset_rw_thread, args=(ip,), daemon=True).start()
    ...
    def _reset_rw_thread(self, ip: str):
        try:
            ...
            if serial: self.log(f"Serial: {serial}")
            else: self.log("Serial not found; attempting anyway.")
    ...
    def on_detect_keys(self):
        ...
        threading.Thread(target=self._detect_keys_thread, args=(ip,), daemon=True).start()
    ...
    def _detect_keys_thread(self, ip: str):
        try:
            ...
            if found:
                ...
                if not self.divider:
                    self.divider = DEFAULT_DIVIDER_GUESS
                if not self.addr_pairs:
                    self.addr_pairs = COMMON_PAIRS[:]
                self._update_labels()
                self.set_progress(100)
                messagebox.showinfo("Keys detected",
            ...
            self.set_progress(0); self.log(f"[!] Detect error: {e!r}")
            messagebox.showerror("Error", str(e))
    ...
    def on_load_wic(self):
        path = filedialog.askopenfilename(
            ...
            initialdir=str(WIC_PATHS[0].parent if WIC_PATHS[0].parent.exists() else Path.home()),
        )
        if not path:
            for p in WIC_PATHS:
                if p.exists(): path = str(p); break
        if not path:
            messagebox.showwarning("Not found", "No WIC log selected/found."); return

        parsed = parse_wic(Path(path))
        if not parsed:
            messagebox.showwarning("No data", "Could not extract keys from the log."); return

        if parsed.password: self.password = parsed.password
        if parsed.write_key: self.write_key = parsed.write_key
        if parsed.oids_seen and not self.addr_pairs:
            self.addr_pairs = [(oid, oid+1) for oid in parsed.oids_seen]
        if not self.divider:
            self.divider = DEFAULT_DIVIDER_GUESS
        self._update_labels()
        self.log(f"[OK] Loaded WIC log. pw={self.password} write_key_len={len(self.write_key or [])} "
    ...
    def on_read_counters(self):
        ...
        if not self.password:
            messagebox.showwarning("Keys required",
        ...
        threading.Thread(target=self._read_counters_thread, args=(ip,), daemon=True).start()
    ...
    def _read_counters_thread(self, ip: str):
        try:
            ...
            if ok_count:
                div = self.divider or DEFAULT_DIVIDER_GUESS
                ...
                self.log("No readable pairs found; need correct keys/addresses.")

            self.set_progress(100)
            messagebox.showinfo("Done", "Read complete. See Log for details.")
        except Exception as e:
            self.set_progress(0); self.log(f"[!] Read error: {e!r}")
            messagebox.showerror("Error", str(e))
    ...
    def on_perm_reset(self):
        ...
        if not preset_writes:
            preset_writes = {20:0,21:0,22:0,23:0,24:0,25:0,28:0,29:0,30:0,34:0,46:94,47:94,60:94,61:94}

        tgt_pct = clamp(float(self.target_pct_var.get()), 0.0, 90.0)
        div = self.divider or DEFAULT_DIVIDER_GUESS
        raw_val = int(round(tgt_pct * div))  
        lo = raw_val & 0xFF; hi = (raw_val >> 8) & 0xFF
        ...
        if not messagebox.askyesno(
            ...
            f"Target USED % after reset: {tgt_pct:.2f}% (raw {raw_val}).\n\n"
            "Make sure pads/kit are serviced. Continue?"
        ):
            return

        threading.Thread(target=self._perm_reset_thread, args=(ip,writes), daemon=True).start()
    ...
    def _perm_reset_thread(self, ip: str, writes: Dict[int,int]):
        try:
            ...
            for addr, value in writes.items():
                ok = asyncio.run(eeprom_write_byte(ip, self.snmp, self.password, addr, 0,
                                                   value & 0xFF, self.write_key))
                done += 1
            ...
            if errors == 0:
                messagebox.showinfo("Permanent reset complete",
                                    "Reset Complete - Please Turn Power to Printer Off and Back On")
                self.log("=== Permanent reset complete. Power-cycle the printer. ===")
            else:
                messagebox.showwarning("Completed with errors",
                                       f"Writes attempted: {total}, failed: {errors}.\n"
                                       "See Log. You may need exact model keys/addresses.")
        except Exception as e:
            self.set_progress(0); self.log(f"[!] EEPROM reset error: {e!r}")
            messagebox.showerror("Error", str(e))
    ...
    def on_status(self):
        ...
        threading.Thread(target=self._status_thread, args=(ip,), daemon=True).start()
    ...
    def _status_thread(self, ip: str):
        try:
            ...
            if not data:
                self.set_progress(0); self.log("[!] No ST2 data.")
                messagebox.showwarning("No data", "Printer did not return ST2 status."); return

            parsed = parse_st2_frame(data)
            self.log("ST2 parsed summary:")
            for k,v in parsed.items():
                if k == "raw_hex": continue
                self.log(f"  {k}: {v}")

            top = tk.Toplevel(self); top.title("Printer Status (ST2)")
            txt = scrolledtext.ScrolledText(top, width=100, height=30)
            txt.pack(fill="both", expand=True)
            txt.insert("end", _dumps(parsed)); txt.config(state="disabled")

            self.set_progress(100)
        except Exception as e:
            self.set_progress(0); self.log(f"[!] ST2 error: {e!r}")
            messagebox.showerror("Error", str(e))
